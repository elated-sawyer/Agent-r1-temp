...
class MLPModel(object):
    def __init__(self, state_path, template_path, device=-1, fp_dim=2048, realistic_filter=True):
    ...
    def run(self, mol_node, topk=10, backward=True, sample_mode=None, test=False):
        ...
        preds, preds_reference, idx = self.forward_topk(arr, topk=50)
        ...
        # print()
        # print(topk)
        # print(len(scores_reference))
        _, idx_topk = torch.topk(torch.tensor(scores_reference), min(topk, len(scores_reference)))

        sorted_idx_topk = sorted(idx_topk)
        reactants_topk = []
        scores_topk = []
        scores_reference_topk = []
        templates_topk = []
        templates_idx_topk = []
        for sidx in sorted_idx_topk:
            reactants_topk.append(reactants[sidx])
            scores_topk.append(scores[sidx])
            scores_reference_topk.append(scores_reference[sidx])
            templates_topk.append(templates[sidx])
            templates_idx_topk.append(templates_idx[sidx])

        scores_topk_norm = [s / sum(scores_topk) for s in scores_topk]
        scores_reference_topk_norm = [s / sum(scores_reference_topk) for s in scores_reference_topk]


        return {'reactants':reactants_topk,
                'scores' : scores_topk_norm,
                'scores_reference' : scores_reference_topk_norm,
                'template' : templates_topk,
                'templates_idx': templates_idx_topk}
