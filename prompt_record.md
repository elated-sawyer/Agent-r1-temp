...
we would collect high quality policy model self generated rollout trajectories as the sft data, since the trajectories generated by other models would mismatch the policy model's, which could make damage on the policy model training

so I would create another git branch, e.g., 'sft_data_collection'. there are several functions need to be improved or added to the current project.
- 