...
def step(env: 'ToolEnvRetro', action_text: str):
    """
...
class ToolEnvRetro:
    ...
    def __init__(self, tools: List[Tool] = None, max_turns: int = 10, maxstep: int = 50, topk: int = 5, shuffle: bool = False):
        """
...
{{"name": <function-name>, "arguments": <args-json-object>}}
    ...
    def _process_action_call(self, value):
        ...
        if eval(value_list[0]) != self.current_state:
            error_msg = f"Parameter molecule has invalid value, should be {self.current_state}-0 ~ {self.current_state}-{len(unsolved_list)-1} ."
            is_valid = False
            return error_msg, is_valid
        if eval(value_list[1]) >= len(unsolved_list):
            error_msg = f"Parameter molecule has invalid value, should be {self.current_state}-0 ~ {self.current_state}-{len(unsolved_list)-1} ."
            is_valid = False
            return error_msg, is_valid
    ...
    def _process_action_select(self, value):
        ...
        if eval(value_list[0]) != self.current_state:
            ...
            is_valid = False
            return error_msg, is_valid
        if eval(value_list[1]) != self.current_molid:
            error_msg = f"Parameter reaction has invalid value, should be {self.current_state}-{self.current_molid}-0 ~ {self.current_state}-{self.current_molid}-{len(self.reaction_dict[reactionid])-1} ."
            is_valid = False
            return error_msg, is_valid
        if eval(value_list[2]) >= len(self.reaction_dict[reactionid]):
            error_msg = f"Parameter reaction has invalid value, should be {self.current_state}-{self.current_molid}-0 ~ {self.current_state}-{self.current_molid}-{len(self.reaction_dict[reactionid])-1} ."
            is_valid = False
            return error_msg, is_valid
    ...
    def wrap_tool_args(self, tool_name: str, tool_args: Dict) -> Tuple[Dict, bool, str]:
        if tool_name == self.tools[0].name:
            ...
            if not is_valid:
                ...
                current_mol = self.unsolved_dict[str(self.current_state)][eval(value_list[1])]
    ...
    def _update_state_variables_message(self, tool_name, tool_args, env_tool_args, result):
        if tool_name == self.tools[0].name:
            try:
                ...
                self.current_molid = eval(tool_args['molecule'].split('-')[1])
                if reaction_list:
                    if tool_args['molecule'] not in self.reaction_dict:
                        self.reaction_dict[tool_args['molecule']] = deepcopy(reaction_list)
                    ...
                    for idy, reaction in enumerate(reaction_list):
                        ...
                        current_message += ' >> ' + env_tool_args['molecule']
            ...
            try:
                ...
                value_list = value.split('-')
                reactionid = str(self.current_state) + '-' + str(self.current_molid)
                current_reaction = self.reaction_dict[reactionid][eval(value_list[2])]
    ...
    def copy(self, target):
        ...
        """
        env = ToolEnvRetro(tools=self.tools, max_turns=self.max_turns, maxstep=self.maxstep, topk=self.topk, shuffle=self.shuffle)
        env.target = target
