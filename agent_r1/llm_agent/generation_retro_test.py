...
from agent_r1.tool.tool_env_retro import ToolEnvRetro, step
...
class ToolGenerationManager:
    ...
    def _process_tool_call(self, responses_str) -> Tuple[List[str], List[bool]]:
        ...
        def process_single_response(resp):
            ...
            if match or '</tool_call>' in resp:
                resp = resp.split(self.config.tool_call_end)[0] + self.config.tool_call_end
                return resp + self.tokenizer.eos_token, True

            if '<tool_call>' in resp:
                resp = resp.split(self.tokenizer.eos_token)[0] + self.config.tool_call_end
                return resp + self.tokenizer.eos_token, True
            
            return resp + self.tokenizer.eos_token, True  # No tool call found
    ...
    def _execute_tool_calls(self, response_strs: List[str], 
        ...
        tool_rewards = [-1] * len(response_strs)
        ...
        for i, (resp, env, active) in enumerate(zip(response_strs, envs, active_list)):
            ...
            tool_rewards[i] = env.rewards[-1]     
        return tool_responses, tool_rewards
    ...
    def run_llm_loop(self, gen_batch, envs: List[Any] = None,
        ...
        for step in range(self.config.max_turns):
            ...
            if not active_mask.sum():
                break
            assert active_mask.sum() == active_mask.shape[0], 'unexpected error'
            ...
            if self.config.use_batch_tool_calls:
                ...
                tool_responses, tool_rewards = self._execute_tool_calls(responses_str, envs, active_mask)
            

            for jj in range(len(tool_rewards)):
                if tool_rewards[jj] < 0:
                    tool_responses[jj] = ""
                    responses_ids[jj,:] = self.tokenizer.pad_token_id
            ...
            for jj in range(len(tool_rewards)):
                if tool_rewards[jj] <= 0:
                    tool_responses_ids[jj,:] = self.tokenizer.pad_token_id
