...
from .metric_utils_retro import compute_data_metrics, compute_throughout_metrics, compute_timing_metrics, reduce_metrics
...
from agent_r1.llm_agent.generation_retro import ToolGenerationManager, ToolGenerationConfig
...
class RayAgentTrainer(object):
    ...
    def __init__(self,
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            self.use_critic = True # 可以改成false
    ...
    def _balance_batch(self, batch: DataProto, envs, metrics, logging_prefix='global_seqlen'):
        ...
        new_envs = deepcopy(envs)
        for i in range(len(new_envs)):
            envs[i] = new_envs[global_idx[i]]
    ...
    def _compute_process_rewards(self, batch, envs, reward_tensor) -> torch.Tensor:
        ...
        start_tag = "</tool_response><|im_end|>\n<|im_start|>assistant\n<think>"
        end_tag = "<|im_end|>\n<|im_start|>user\n<tool_response>"
        ...
        for i, (response, env) in enumerate(zip(responses, envs)):
            ...
            # ground_truth_length = len(json.loads(ground_truth)) * 2
            # print("***********")
            ...
            end_positions = [m.start() for m in re.finditer(re.escape(end_tag), response)]
            tool_call_ends = []
            j = 0
            k = 0
            curr_start = 0
            while True:
                if j >= len(end_positions):
                    break
                curr_end = end_positions[j]
                ...
                if j >= len(end_positions):
                    break
                tool_call_ends.append(end_positions[j])
                if k >= len(start_positions):
                    break
                curr_start = start_positions[k]
                while curr_start <= tool_call_ends[-1]:
                    k += 1
                    if k >= len(start_positions):
                        break
                    curr_start = start_positions[k]
            ...
            for tool_idx, end_pos in enumerate(tool_call_ends):
                ...
                prefix = response[:end_pos]

                token_pos = len(self.tokenizer.encode(prefix, add_special_tokens=False)) - 1

                if tool_idx >= len(env_rewards):  # Safety check
                    print(f"[DEBUG] The number of tool call {tool_idx} is larger than rewards {len(env_rewards)}.")
                    process_rewards[i, token_pos] = -0.1
                    break
                ...
                # if not first_end and tool_idx < len(tool_call_ends) - 1:
                #     prefix_next = response[:tool_call_ends[tool_idx+1] + len(self.config.tool.tool_call_end)]
                #     if 'You have successfully found a pathway to synthesize the target molecule' in prefix_next:
                #         first_end = True
                #         if token_pos < valid_response_length:
                #             if tool_idx+1 <= ground_truth_length:
                #                 process_rewards[i, token_pos] += 0.6 + (ground_truth_length - tool_idx-1)/ground_truth_length/2
                #             else:
                #                 process_rewards[i, token_pos] += 0.6 - min((tool_idx+1 - ground_truth_length)/ground_truth_length/2, 0.5)

                            # process_rewards[i, token_pos] += 1.0/tool_idx+0.1
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                envs = [self.env.copy(batch.non_tensor_batch['target'][ii]) for ii in range(len(batch))]
                ...
                with _timer('step', timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        raise ValueError("adv_estimator cannot be remax")
                    ...
                    if self.config.trainer.balance_batch:
                        self._balance_batch(batch, envs, metrics=metrics)
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        reward_tensor, end_lst, answer_lst, format_lst = self.reward_fn(batch, envs)
                        ...
                        batch.batch['end_scores'] = torch.tensor(end_lst)
    ...
    def _create_loss_mask(self, batch: DataProto, metrics: dict) -> Tuple[DataProto, dict]:
        ...
        batchsize = batch.batch['responses'].shape[0]
        ...
        metrics.update({
            ...
            'state_tokens/mean': loss_mask.sum().item() / batchsize,
